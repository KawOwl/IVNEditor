/**
 * ⚠️ 已废弃 (DEPRECATED) — 一次性历史升级脚本，已经在线上执行过了
 *
 * 保留原因：
 *   - 作为历史记录，记录那次老 schema 到新 schema 的升级过程
 *   - 新环境不需要跑这个——直接 `bun run start` 就行，
 *     drizzle 会从 0000_boring_galactus.sql baseline 全量建表
 *
 * 什么时候可以彻底删除：
 *   - 确认所有环境都已经走过标准 drizzle 迁移流程
 *   - 没有人再需要参考这段一次性迁移逻辑
 *
 * ─────────────────────────────────────────────────────────────
 * 以下为历史文档：
...
async function migrate() {
  ...
  if (!(await tableExists('playthroughs'))) {
    ...
    console.log('  cd apps/server && DATABASE_URL=... bun drizzle-kit push');
    return;
  }
