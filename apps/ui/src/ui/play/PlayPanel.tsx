...
// ============================================================================
// FeedbackModal — 反馈两阶段（PFB.1 + PFB.2）
//
// 顶层根据 useAuthStore.kind 路由：
//   anonymous → RegisterForm（邮箱+密码+6 题画像 → POST /api/auth/register
//               → checkMe → 切到 FeedbackForm）
//   registered/admin → FeedbackForm（5 题问卷 → POST /api/feedback）
//
// 所有选项原文必须与后端常量同步：
//   - 反馈 5 题 ↔ apps/server/src/routes/feedback.mts FEEDBACK_OPTIONS
//   - 注册画像 ↔ apps/server/src/routes/auth.mts PROFILE_OPTIONS
...
function FeedbackModal({
  ...
  // anonymous → register；registered/admin → feedback
  // 提交注册成功后由 RegisterForm 调 checkMe + onDone 把 step 推到 'feedback'
  const [step, setStep] = useState<'register' | 'feedback'>(
    kind === 'anonymous' ? 'register' : 'feedback',
  );

  // kind 在 modal 打开期间变化（极少见，但 logout 会触发）→ 重置 step
  useEffect(() => {
    if (kind !== 'anonymous' && step === 'register') setStep('feedback');
  }, [kind, step]);
      ...
      >
        {checking ? (
          <div className="flex-1 flex items-center justify-center px-5 py-12">
            <p className="text-sm text-zinc-400">校验登录状态中…</p>
          </div>
        ) : step === 'register' ? (
          <RegisterForm onClose={onClose} onDone={() => setStep('feedback')} />
        ) : (
...
// ============================================================================
// RegisterForm — 邮箱 + 密码 + 6 题画像 → POST /api/auth/register
// ============================================================================

function RegisterForm({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const toggleHobby = useCallback((opt: string) => {
    setHobbies((prev) => {
      ...
      if (prev.length >= HOBBY_MAX_PICK) return prev; // 已选满 2 个，再点不加
      return [...prev, opt];
    });
  }, []);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // 注册成功 → 刷新 auth-store 的 identity → 切到反馈
      await checkMe();
      onDone();
    } catch (err) {
    ...
    <>
      <div className="flex-none px-5 py-4 border-b border-zinc-800">
        ...
        <p className="text-[11px] text-zinc-500 mt-1">
          邮箱+密码用于以后再来看你自己的反馈；下面 6 题用户画像帮我们分析样本。
        </p>
      </div>
