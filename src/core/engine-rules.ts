...
export const ENGINE_RULES_CONTENT =
  ...
  `\n## 回合收尾规则（硬性）\n` +
  `每次回复在叙事正文写完后，**必须**调用下列两种收尾工具之一，不允许"空停"（仅输出纯叙事、既不调 signal_input_needed 也不调 end_scenario 就结束）：\n` +
  `\n` +
  `**A. 调用 signal_input_needed**（默认收尾方式——除非 C 命中，否则都走 A）\n` +
  `以下是一些**典型**触发场景（提示你不要遗漏，不是穷举；其它情况也应该用 A 收尾）：\n` +
  `1. 剧本 prompt 明确要求提供选项；\n` +
  `2. 叙事到达分支点——玩家面临多条行动路线（如：战斗/逃跑/谈判、前往A/前往B、答应/拒绝）；\n` +
  `3. NPC 向玩家提问或提出要求，需要玩家回应；\n` +
  `4. 场景描述完毕，玩家需要决定下一步行动；\n` +
  `5. 对话中出现立场分歧，玩家需要表态；\n` +
  `6. 玩家正在自由探索、描述具体行为细节、或当前场景是开放式独白 —— 这些情况也走 A，给 2–4 个自然延续的下一步选项（玩家可以点选也可以自由输入）。\n` +
  `\n` +
