...
## 当前状态
v2.6 剧本版本管理 + 编辑器试玩走后端全部完成（6.1-6.6）。玩家侧和编辑器侧都只走后端 scripts + script_versions 双表路径，前端 IndexedDB 已退役（LocalBackupGate 作为过渡期 modal，检测到遗留本地数据时强制用户先导出备份再清理）。Langfuse trace 能区分 production / editor-playtest。
