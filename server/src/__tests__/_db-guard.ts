 ...
 * DB 测试安全锁
 *
 * 问题：server/src/__tests__/ 里的 cleanTables() 对整张 users / scripts /
 ...
 *   3. 或 cp server/.env server/.env.test 改 DATABASE_URL，然后用
...
export async function assertTestDatabase(): Promise<void> {
  ...
  if (!/test/i.test(name)) {
    throw new Error(
      ...
      `       或者 cp server/.env server/.env.test 改指向 ivn_test，\n` +
